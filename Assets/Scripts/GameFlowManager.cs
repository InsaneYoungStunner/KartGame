...
public class GameFlowManager : MonoBehaviour
{
    ...
    public string complicateName = "ComplicateScene";
    ...
    public KartMovement playerKart;
    ...
    public static bool complicated = false;
    ...
    void Start()
    {
        if (autoFindKarts)
        {
            karts = FindObjectsOfType<KartMovement>();
            Debugger.Log("karts.Length {0} ", karts.Length);
            if (karts.Length > 0)
            {
                if (!playerKart) playerKart = karts[0];
            }
            DebugUtility.HandleErrorIfNullFindObject<KartMovement, GameFlowManager>(playerKart, this);
        }
        ...
        complicated = false;
        Debug.Log(JsonUtility.ToJson(m_TrackManager));
        ...
        m_ScoreManager.StopRace();
        m_CoinManager.StopRace();
        m_TrackManager.StopRace();
        foreach (KartMovement k in karts)
        {
			k.DisableControl();
        }
    ...
    void StartRace() {
        ...
        m_ScoreManager.StartRace();
        m_CoinManager.StartRace();
        m_TrackManager.StartRace();
    }
    ...
    void Update()
    {
        score = m_ScoreManager.currentScore;
    ...
    private void stopTrack()
    {
        ...
        {
            m_TrackManager.StopRace("local player", m_ScoreManager.currentScore, 0, m_CoinManager.currentCount, currentTime.ToString(), m_TimeManager.TotalTime - m_TimeManager.TimeRemaining);
        }
        else
        {
            m_TrackManager.StopRace(myPlayerInfo.Name, m_ScoreManager.currentScore, 0, m_CoinManager.currentCount, currentTime.ToString(), m_TimeManager.TotalTime - m_TimeManager.TimeRemaining);
        }
